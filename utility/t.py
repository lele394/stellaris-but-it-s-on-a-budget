...
def start():
    ...
    try:
        ...
        ConsolePrint("save file not detected, please create an empty file called save.py", "s")
        quit()
        f.close()
...
def OnPlayerAdd(name, system=(-1)): #gives a random non pirate system if no system number  is precised
    if system == (-1):
...
def CreateMovingFleet(origin, destination, power): #takes destination and origin star number and power, adds fleet to moving fleet, substract power from system raise error if not enough fleet power in the system
    ...
    try:
        if power > system_fleet_power:
            raise ValueError("fleet power is superior to system fleet power, fleet cancelled")
        else:
            galaxy[origin][3] += -int(power)
            __main__.moving_fleets.append(fleet)
            ConsolePrint("created fleet of "+ str(power)+ " power from "+ str(origin)+ " to "+ str(destination), "s")
    except ValueError as err:
        ConsolePrint(err,"s")
        pass
